...
flight_times <- data.frame(country    = c("EU", "USA", "RR"),
                           dur_flight = c(2/24, 8/24, 1/(24*60*60))) 
# flights from fake country RR (for rate ratios) arrive 1 second later
...
calc_outcomes <- function(x, dat_gam){
  ...
  x <- mutate(x,
              zeroth_test_p =
                c(stats::predict(object  = dat_gam,
                                 type    = "response",
...
make_release_figure <- function(x,
  ...
  x %<>% mutate(time_in_iso_ = first_test_delay + second_test_delay_,
                time_in_iso_  = factor(time_in_iso_, levels = sort(unique(time_in_iso_))))
  
  # how to do presymptomatic
  # don't do a no_dots here
  ...
  facet_lhs <- formula.tools::lhs.vars(faceting)
  facet_rhs <- formula.tools::rhs.vars(faceting)
  facet_rhs <- grep(x = facet_rhs,
                    pattern = c("^(stringency|tests)$"), value = T, fixed = F, invert = T)
  
  
  if (length(facet_rhs > 0)){
    if (all(facet_rhs == ".")){
      facet_rhs <- character(0)
    }
  }
  
  facet_rhs <- c(facet_rhs, 
                 "tests_",
                 "stringency")
  
  
  faceting <- as.formula(paste(paste(facet_lhs, collapse = " + "),
                               paste(facet_rhs, collapse = " + "),
                               sep = " ~ "))
  
  facet_vars_dy <- no_dots(facet_vars)
    
  
  dy <- select(ungroup(x), !!!facet_vars_dy,
               `97.5%`, `75%`) %>%
    gather(key, value, -c(!!!facet_vars_dy)) %>%
    filter(value < Inf) %>%
    group_by_at(.vars = vars(-value)) %>%
    filter(value == max(value)) %>%
    distinct %>%
    {left_join(select(., -value) %>% 
                 as.list %>%
                 map(unique) %>%
                 expand.grid, .)} %>%
    spread(key, value) %>%
  ...
  facet_lhs_vars_dy <- no_dots(facet_lhs)
  
  ymax <- x %>% group_by(!!!facet_lhs_vars_dy) %>% 
  ...
  x %<>% mutate(tests_ = fct_collapse(.f = factor(tests),
                                      `Zero or one tests` = c("Zero", "One"),
                                      `Two tests` = "Two"),
                pre_board_screening = as.character(pre_board_screening)) %>%
    replace_na(list(pre_board_screening = "NA"))
  
  figure <-  
    ggplot(data=x, aes(x = time_in_iso_, 
                       y = `50%`, 
                       color = tests)) +
    ...
    ggplot2::labs(x = xlab,
                  y = ylab) +
    scale_alpha_manual(name = "Days until first of two tests",
                       values = c(1),
                       breaks = c(" ")) +
    xlab("Days in quarantine until final test (if any)") +
    guides(alpha = guide_legend(override.aes = list(label = "0-3", size = text_size)),
           color = guide_legend(override.aes = list(label = "")))
  
  figure <- figure + facet_nested(facets = faceting,
                                  nest_line = T, scales = "free_x", space = "free",
                                  labeller = labeller(pre_board_screening = pre_board_labels,
                                                      country = function(x){paste("Origin:",x)}))
  # }
...
make_plots <- function(
  ...
  all_grouping_vars <- no_dots(all.vars(faceting))
  ...
  if (days){
    ...
    figS3 <- figS3A + figS3B + plot_layout(nrow = 1,
                                           guide = "collect") +
    ...
    return(list(figure = figS3,
                data   = list(number = figS3A_data,
                              days   = figS3B_data)))
